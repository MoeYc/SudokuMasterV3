...
const PlayGrid = () => {
  ...
  const validateAnswer = () => {
    if (!isCompleted()) {
      message.warn('尚有单元格未完成。如果您希望检查目前输入的正确性，请点击<检查残局>按钮。');
      return;
    }
    ...
    <Spin spinning={loading} size="large" tip="加载中...">
      <div style={{ textAlign: 'center', margin: '0px auto', height: 444 }}>
        ...
        <div className="gridFg">
                  ...
                  return (
                    <tr key={'tr' + iRow} className="grid">
                      {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((iCol) => {
                        return <Cell key={'cell' + (iRow * 9 + iCol)} index={iRow * 9 + iCol} />;
                      })}
                    </tr>
                  );
                })}
          ...
          &nbsp;&nbsp;
          <Button onClick={() => validateAccess() && setInputVisible(true)}>自定义盘</Button>
          &nbsp;&nbsp;
          ...
          <audio id="audio_player" loop autoPlay>
            <source src={require('./musics/theLanguageOfPipa.mp3')} />
          </audio>
          <CreateGridForm
