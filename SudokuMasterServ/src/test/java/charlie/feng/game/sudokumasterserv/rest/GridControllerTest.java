...
public class GridControllerTest {
    ...
    @BeforeEach
    void setUp() {
        mvc = MockMvcBuilders.standaloneSetup(gridController).addPlaceholderValue("charlie.feng.aa.apiPrefix", "/api").build();
    }
    ...
    void testResolveGrid() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.get("/api/grid/000000018948007050000008020053702000009000000000901430090600000030500876060000000/resolve"))
        ...
        //Test grid length
        mvc.perform(MockMvcRequestBuilders.get("/api/grid/12345/resolve"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("resolved").value(false))
                .andExpect(jsonPath("msg").value("数独盘面由81位数字组成。空格请输入0。"));

        //Test non digital char
        mvc.perform(MockMvcRequestBuilders.get("/api/grid/12345678912345678912345678912345678912345678912345678912345678912345678912345678a/resolve"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("resolved").value(false))
                .andExpect(jsonPath("msg").value("数独盘面由81位数字组成。空格请输入0。"));

        //Test invalid grid id;
        mvc.perform(MockMvcRequestBuilders.get("/api/grid/003000802000700090100090400010609700050207060607508040001060009060004000508000030/resolve"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("resolved").value(false))
    ...
    void testResolvePosition() throws Exception {
        String baseUrl = "/api/position/000000018948007050000008020053702000009000000000901430090600000030500876060000000/";
