...
public class JwtAuthenticationRestController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody JwtTokenRequest authenticationRequest)
        ...
        return ResponseEntity.ok(new JwtTokenAndUserInfoResponse(token, userDetails.getUsername(), fullname, userDetails.getAuthorities(), "success"));
    }
    @RequestMapping(value = "${jwt.path.logoff}", method = RequestMethod.POST)
    public ResponseEntity<?> logoff(HttpServletRequest request) {
        //We may cleanup session, and/or add Jwt token into redis blacklist
    ...
    @RequestMapping(value = "${jwt.path.currentUser}", method = RequestMethod.GET)
    public ResponseEntity<?> currentUser(HttpServletRequest request) {
        ...
        if (StringUtils.hasLength(authToken)) {
            final String token = authToken.substring(7);
            String username = jwtTokenUtil.getUsernameFromToken(token);
            JwtUserDetails userDetails = (JwtUserDetails) jwtJPAUserDetailsService.loadUserByUsername(username);
